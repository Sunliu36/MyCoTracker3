...
if __name__ == "__main__":
    ...
    #[Shawn] Check using NVIDIA GPU
    print(f"✅ Using device: {video.device}, torch device count = {torch.cuda.device_count()}")
    print(f"🧠 CUDA device name: {torch.cuda.get_device_name(torch.cuda.current_device())}")

    #[Shawn] only track query points
    queries_point = torch.tensor([[
    [0., 690., 409.],  # point tracked from the first frame
    [0., 723., 445.], # frame number 10
    [0., 563., 456.], # ...
    [0., 613., 517.]
    ]], dtype=torch.float32).to(video.device)
